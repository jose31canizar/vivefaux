  ...
  render() {
    ...
    return (
      <SmoothScroll className={`panel`} to={this.props.title.toLowerCase()}>
        <div
          className={`left-panel ${this.state.panelStyle}`}
          onMouseDown={this.togglePanelFromClick}
        >
          <Cross
        ...
        <div className={`left-panel-text ${this.state.panelStyle}`}>
          <h1 className="label-title" data-text={self.props.title}>
            {self.props.title}
          </h1>
          <p className="label-description" data-text={self.props.description}>
            {self.props.description}
          </p>
          {/* <Audio/> */}
        </div>
        <div
          className={`right-panel-text ${this.state.panelStyle}`}
          dangerouslySetInnerHTML={{ __html: this.props.releases }}
        />
      </SmoothScroll>
    );
  }
